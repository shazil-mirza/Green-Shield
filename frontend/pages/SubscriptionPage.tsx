...
import { CheckCircle, CreditCard, Settings, Crown, ShieldAlert } from 'lucide-react';
...
const SubscriptionPage: React.FC = () => {
  ...
  const handleUpgradeToPremium = async () => {
    ...
    try {
      ...
      setError(err.data?.message || err.message || 'Failed to start subscription process. Please try again.');
      setIsLoading(false);
    }
  };
  ...
  const handleManageSubscription = async () => {
    ...
    try {
      ...
      setError(err.data?.message || err.message || 'Failed to open customer management portal. Please try again.');
      setIsLoading(false);
    }
  };
  ...
  if (!user) {
    return <div className="text-center py-10">
        <p className="text-xl">Please <Link to="/login" className="text-primary underline">log in</Link> to manage your subscription.</p>
    </div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
       ...
       {user.stripeSubscriptionStatus && user.stripeSubscriptionStatus !== 'active' && user.stripeSubscriptionStatus !== 'trialing' && !isUserAdmin && (
            <div className="mt-6 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 rounded-md text-center">
                <p>Your subscription status is currently: <strong className="capitalize">{user.stripeSubscriptionStatus.replace(/_/g, ' ')}</strong>.</p>
