...
const HistoryPage: React.FC = () => {
  ...
  const fetchHistory = useCallback(async () => {
    if (user) {
      ...
      setError(null);
      try {
        const data = await apiService.get<DetectionResult[]>('/api/detections');
        setHistory(data || []);
      } catch (err: any) {
        console.error("Failed to fetch history:", err);
        setError(err.response?.data?.message || "Could not load your detection history.");
        setHistory([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleDelete = async (id?: string) => {
    ...
    try {
      await apiService.delete(`/api/detections/${id}`);
    } catch (err: any) {
      console.error("Failed to delete detection:", err);
      setError(err.response?.data?.message || "Could not delete the detection. Please try again.");
